...
describe UrlParser do
  context 'with all parts' do
    ...
    it 'when instantiated should be a member of the UrlParser class' do ####DONE
      expect(@new_url).to be_a UrlParser
    end

    it 'should have a scheme attribute' do ###DONE
      expect(@new_url.scheme).to eq("http")
    end

    it 'should have a domain attribute' do ###DONE
      expect(@new_url.domain).to eq("www.google.com")
    end

    it 'should have a port attribute with the given port number' do ###DONE
      expect(@new_url.port).to eq("60")
    end

    it 'should have a path attribute' do ###DONE
      expect(@new_url.path).to eq("search")
    end

    it 'should have a query string attribute that should return a hash of query params' do ###DONE
      expect(@new_url.query_string).to be_a(Hash)
      expect(@new_url.query_string).to eq({"q" => "cat", "name" => "Tim"})
    end

    it 'should have a fragment id attribute' do ###DONE
  ...
  context 'with no path' do
    ...
    it 'should return a nil path' do ###DONE
      expect(@new_url.path).to be(NIL)
    end

    it 'should be able to have a query string at the root path' do ###DONE
      expect(@new_url.query_string).to eq({"q" => "cat"})
    end

    it 'should be able to have a fragment id at the root path' do ###DONE
      expect(@new_url.fragment_id).to eq("img=FunnyCat")
    end
  end
